        ...
        {!status?.configured ? (
          ...
          <>
            ...
            {!status.connected && qr ? (
              ...
              <p className="muted">Session active. Scheduled jobs can send to WhatsApp destinations.</p>
            )}
          </>
        )}
      </section>
      <section className="card">
        <h2>Telegram worker</h2>
        <p className="muted">
          Configure TELEGRAM_BOT_TOKEN on telegram-worker. Destinations use the Telegram chat ID as external ID.
        </p>
      </section>
    </div>
  )
